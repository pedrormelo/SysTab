...
exports.login = async (req, res) => {
    ...
    try {
        ...
        // Add both nivel and role for frontend compatibility
        const token = jwt.sign({ idLogin: user.idLogin, nivel: user.nivel, role: user.nivel }, SECRET, { expiresIn: '1d' });
        res.json({ token, usuario: { nome: user.nome, nivel: user.nivel, role: user.nivel } });
    } catch (error) {
